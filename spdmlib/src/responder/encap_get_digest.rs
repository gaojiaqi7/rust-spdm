...
impl<'a> ResponderContext<'a> {
    pub fn encode_encap_request_get_digest(&mut self, encap_request: &mut Writer) {
        let encap_request_offset = encap_request.used();
        ...
        let _ = self
            .common
            .append_message_mut_b(&encap_request.used_slice()[encap_request_offset..]);
    }
                ...
                match header.request_response_code {
                    SpdmRequestResponseCode::SpdmResponseDigests => {
                        ...
                        if let Some(digests) = digests {
                            debug!("!!! digests : {:02x?}\n", digests);
                            self.common.append_message_mut_b(&encap_response[..used])?;
